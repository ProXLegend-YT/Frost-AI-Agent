...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={copy}
      className="absolute top-2 right-2 text-xs bg-gray-700 hover:bg-gray-600 text-gray-300 px-2 py-1 rounded-md transition-colors"
    >
      {copied ? '✓ Copied' : 'Copy'}
    </button>
  );
      ...
      <div className={`max-w-[85%] ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
        <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed
          ${isUser
            ...
            : 'bg-[#1a1a2e] text-gray-100 rounded-tl-sm border border-[#2a2a40]'}`}>
